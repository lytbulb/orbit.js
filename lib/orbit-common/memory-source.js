...
import { RecordNotFoundException, LinkNotInitializedException } from './lib/exceptions';
...
var MemorySource = Source.extend({
  ...
  _fetchLinked: function(type, id, link, options) {
    ...
    if (linkValue === OC.LINK_NOT_INITIALIZED) throw new LinkNotInitializedException(type, id, link);
    ...
    return new Orbit.Promise(function(resolve, reject) {
      ...
      if (record) {
        ...
        if (record.__rel) {
          ...
          if(relId === OC.LINK_NOT_INITIALIZED) {
            reject(new LinkNotInitializedException(type, id, link));
          }
